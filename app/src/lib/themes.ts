...
export interface Theme {
  id: string;
  label: string;
  /** Light UI: flips native color-scheme. */
  light?: boolean;
...
/** Pre-render apply of the last used theme — avoids a flash of the default. */
...
export function cachedThemeId(): string {
  try {
    return localStorage.getItem(CACHE_KEY) ?? "dark";
  } catch {
    return "dark";
  }
}
