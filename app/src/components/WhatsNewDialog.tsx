...
// последние релизы, невиданные помечены (см. lib/whatsNew).
import { openUrl } from "@tauri-apps/plugin-opener";
import { ExternalLink, Sparkles, X } from "lucide-react";
import { useEffect, useState } from "react";
import { RELEASES_PAGE_URL, collectWhatsNew, type ReleaseNote } from "../lib/whatsNew";
import { Button, IconButton, Overlay } from "./ui";

/** Строки changelog: "- пункт" → буллет, остальное — абзац. */
    ...
    <Overlay onClose={close} closeOnEsc className="items-center bg-black/60">
      <div className="w-[30rem] max-w-[92vw] self-center rounded-lg border border-zinc-700 bg-zinc-900 p-4 shadow-2xl">
          ...
          {notes.map((n) => (
            <div key={n.version}>
              <div className="mb-1 flex items-center gap-2">
                <span className="text-[11px] font-semibold text-sky-400">v{n.version}</span>
                {n.fresh && (
        ...
        <div className="mt-4 flex items-center justify-between">
          <Button title="Открыть страницу релизов в браузере" onClick={() => void openUrl(RELEASES_PAGE_URL)}>
