...
func setCommand(args []string) int {
	...
	existed := store.Put(keys, key, val)
...
// otpCommand печатает одноразовый TOTP-код из сохранённого seed. Код живёт
// секунды, поэтому показывать его безопасно даже в чате.
