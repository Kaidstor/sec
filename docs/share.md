...
(`text`/`file`/`pack`), содержимым и — для файлов — исходным именем и правами.
То есть даже **имя файла сервер не видит**: `server.p12` зашифрован ровно так
же, как его содержимое. Пак (`sec share <proj> --all` или `--only A,B`) — тот
же конверт со списком записей: имя проекта и все ключи с значениями едут одним
шифроблобом, страница раскрытия отдаёт их списком с покопийным «скопировать»,
готовым `.env` и отдельным скачиванием файловых ключей.
